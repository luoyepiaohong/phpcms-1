    ...
    <div class="content">
        <table width="100%" cellspacing="1" cellpadding="0">
			  <tr>
				...
				<td><input name="adminpath" id="adminpath" type="text" placeholder="设置后台登录地址" value="" class="input-text" /><br>后台登录地址设置同文件夹命名规则，可为空，为空不更改后台地址，例如:admin 安装完成后后台登录地址即为 <?php echo $siteurl?>admin</td>
			  </tr>
              <tr>
...
errmsg[7] = '后台登录口地址不能使用CMS默认目录名（admin，api，caches，cms，html，mobile，statics，uploadfile），请重新设置！';

function checkdb() {
    var url = '?step=dbtest&adminpath='+$('#adminpath').val()+'&dbhost='+$('#dbhost').val()+'&dbport='+$('#dbport').val()+'&dbuser='+$('#dbuser').val()+'&dbpw='+$('#dbpw').val()+'&dbname='+$('#dbname').val()+'&tablepre='+$('#tablepre').val()+'&sid='+Math.random()*5;
    if($('#dbhost').val()==''){
