...
class Uploader
{
    ...
    public function __construct($fileField, $config, $type = "upload")
    {
        ...
        $this->attachment_info = $this->upload->get_attach_info(($this->attachment ? (int)$this->attachment : intval(SYS_ATTACHMENT_ATTACH)), (int)$this->config['image_reduce']);
    ...
    private function upFile()
    {
        ...
        // 安全检测
        $rt = $this->upload->_safe_check(trim($this->getFileExt(), '.'), file_get_contents($file["tmp_name"]), 1);
        if (!$rt['code']) {
    ...
    private function upBase64()
    {
        ...
        // 安全检测
        $rt = $this->upload->_safe_check(trim($this->getFileExt(), '.'), $img, 1);
        if (!$rt['code']) {
            $this->stateInfo = $rt['msg'];
            return;
        }
