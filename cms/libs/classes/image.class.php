...
class image {
    ...
    public function thumb($img, $width = 0, $height = 0, $water = 0, $mode = 'auto', $webimg = 0) {
        ...
        if ($img) {
            ...
            if (!$attach) {
                //log_message('error', '图片[id#'.$img.']不存在，thumb函数无法调用');
                return IMG_PATH.'nopic.gif';
            } elseif (!in_array($attach['fileext'], array('gif', 'png', 'jpeg', 'jpg'))) {
                //log_message('error', '图片[id#'.$img.']扩展名不符合条件，thumb函数无法调用');
                return IMG_PATH.'nopic.gif';
            }
        } else {
            $attach = array(
        ...
        if ($this->memory_limit($this->image_info)) {
            //log_message('error', '图片[id#'.$attach['id'].']的URL['.$attach['url'].']分辨率太大导致服务器内存溢出，无法进行缩略图处理，已按原图显示');
            return $attach['url']; // 原样输出
        }
