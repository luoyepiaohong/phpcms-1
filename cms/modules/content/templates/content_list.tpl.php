...
<div class="admin-main layui-anim layui-anim-upbit">
    ...
    <blockquote class="layui-elem-quote">
        ...
        <a href="?m=content&c=content&a=init&catid=<?php echo $catid;?>&pc_hash=<?php echo $pc_hash;?>" class="layui-btn layui-btn-sm<?php if($steps==0 && !isset($_GET['reject'])) echo ' on';?>">
    ...
    <div class="demoTable" id="searchid" style="display:<?php if(!isset($_GET['search'])) echo 'none';?>">
        <?php echo L('addtime');?>：
        <?php echo form::date('start_time',$_GET['start_time'],0,0,'false');?>- &nbsp;<?php echo form::date('end_time',$_GET['end_time'],0,0,'false');?>
                <select id="posids" name="posids"><option value='' <?php if($_GET['posids']=='') echo 'selected';?>><?php echo L('all');?></option>
...
<script type="text/html" id="username">
    {{# if(d.sysadd==0){ }}
    <a href='javascript:;' onclick="omnipotent('member','?m=member&c=member&a=memberinfo&username={{d.deusername}}&pc_hash=<?php echo $_GET['pc_hash'];?>','<?php echo L('view_memberlinfo');?>',1,700,500);">{{d.username}}</a><img src="<?php echo IMG_PATH;?>icon/contribute.png" onmouseover="layer.tips('<?php echo L('member_contribute');?>',this,{tips: [1, '#000']});" onmouseout="layer.closeAll();">
    {{# } else { }}
    {{d.username}}
    {{# } }}
</script>
...
layui.use(['table'], function(){
    ...
    var tableIn = table.render({
        ...
        cols: [[
            ...
            {width: 240, align: 'center', toolbar: '#action',title:'<?php echo L('operations_manage');?>', fixed: 'right'}
        ]],
        limit: 10
    });
    //搜索
