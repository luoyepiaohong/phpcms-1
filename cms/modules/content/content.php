	...
	public function init() {
		...
		if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
			...
			if (IS_POST) {
				...
				if(isset($_POST['posids']) && !empty($_POST['posids'])) {
					$posids = $_POST['posids']==1 ? intval($_POST['posids']) : 0;
					$where .= " AND `posids` = '$posids'";
				}
				$pagesize = $_POST['limit'] ? $_POST['limit'] : 10;
				$datas = $this->db->listinfo($where,'id desc',$_POST['page'],$pagesize);
			...
			for($i=1;$i<=$workflow_steps;$i++) {
				...
				$workflow_menu .= '<a href="?m=content&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&steps='.$i.'&pc_hash='.$pc_hash.'" class="layui-btn layui-btn-sm'.$current.'"><i class="fa fa-check"></i>'.L('workflow_'.$i).$newimg.'</a>';
			}
			if($workflow_menu) {
				$current = isset($_GET['reject']) ? ' layui-btn-danger' : '';
				$workflow_menu .= '<a href="?m=content&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&pc_hash='.$pc_hash.'&reject=1" class="layui-btn layui-btn-sm'.$current.'"><i class="fa fa-check"></i>'.L('reject').'</a>';
			}
	...
	public function public_categorys() {
		...
		if(!empty($this->categorys)) {
			foreach($this->categorys as $r) {
				...
				if($r['type']==1 || $from=='block') {
					if($r['type']==0) {
						$r['vs_show'] = "<a href='?m=block&c=block_admin&a=public_visualization&menuid=".$_GET['menuid']."&catid=".$r['catid']."&type=show' target='".$_GET['from']."_right'>[".L('content_page')."]</a>";
					} else {
						$r['vs_show'] ='';
					}
					$r['icon_type'] = 'file';
					$r['category_edit'] = " <a href='?m=content&c=content&a=add&menuid=".$_GET['menuid']."&catid=".$r['catid']."' target='right'>".L('edit')."</a>";
					$r['add_icon'] = '';
					$r['type'] = 'add';
				} else {
		...
		if(!empty($categorys)) {
			$tree->init($categorys);
				switch($from) {
					case 'block':
						$strs = "<span class='\$icon_type'>\$add_icon<a href='?m=block&c=block_admin&a=public_visualization&menuid=".$_GET['menuid']."&catid=\$catid&type=list' target='".$_GET['from']."_right'>\$catname</a> \$vs_show</span>";
						$strs2 = "<img src='".IMG_PATH."folder.gif'> <a href='?m=block&c=block_admin&a=public_visualization&menuid=".$_GET['menuid']."&catid=\$catid&type=category' target='".$_GET['from']."_right'>\$catname</a>";
					break;

					default:
						$strs = "<span class='\$icon_type'>\$add_icon<a href='?m=content&c=content&a=\$type&menuid=".$_GET['menuid']."&catid=\$catid' target='right'>\$catname</a></span>";
	...
	public function public_crop() {
		...
		if(isset($_POST['filepath'])){
			...
			if (pc_base::load_config('system','attachment_file')) {
				$filename = $this->siteid.'/'.date('Y/md/').date("ymdhis").substr(md5(SYS_TIME.($_POST['filepath']).uniqid()), rand(0, 20), 15);
			} else {
				$filename = date('Y/md/').date("ymdhis").substr(md5(SYS_TIME.($_POST['filepath']).uniqid()), rand(0, 20), 15);
			}
			//$new_file = date('Y/md/').'thumb_'.$w.'_'.$h.'_'.basename($_POST['filepath']);
			$filetype = fileext($_POST['filepath']);
			$fileinfo = $image->crops(CMS_PATH.$_POST['filepath'], SYS_UPLOAD_PATH.$filename.'.'.$filetype, $w, $h, $x, $y);
			//$fileinfo = $image->crops(CMS_PATH.$_POST['filepath'], SYS_UPLOAD_PATH.$new_file, $w, $h, $x, $y);
			if($fileinfo){
				...
				$uploadedfile['filename'] = file_name($_POST['filepath']);
				$uploadedfile['filesize'] = $fileinfo['size'];
				$uploadedfile['fileext'] = $filetype;
				$aid = $this->att_db->api_add($uploadedfile);
				$this->upload_json($aid,SYS_UPLOAD_URL.$filename.'.'.$filetype,file_name($_POST['filepath']),format_file_size($fileinfo['size']));
