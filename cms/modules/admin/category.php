...
class category extends admin {
		...
		if (IS_POST) {
			...
			if(!empty($result)) {
				foreach($result as $r) {
					...
					$rs['manage'] = '<a href="javascript:addedit(\'?m=admin&c=category&a=add&parentid='.$r['catid'].'&menuid='.$_GET['menuid'].'&s='.$r['type'].'&pc_hash='.$_GET['pc_hash'].'\', \''.L('add_sub_category').'\')" class="layui-btn layui-btn-xs"><i class="fa fa-plus"></i> '.L('add_sub_category').'</a><a href="javascript:addedit(\'?m=admin&c=category&a=edit&catid='.$r['catid'].'&menuid='.$_GET['menuid'].'&type='.$r['type'].'&pc_hash='.$_GET['pc_hash'].'\', \''.L('edit').'\')" class="layui-btn layui-btn-xs"><i class="fa fa-edit"></i> '.L('edit').'</a><a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del"><i class="fa fa-trash-o"></i> '.L('delete').'</a><a href="?m=admin&c=category&a=remove&catid='.$r['catid'].'&menuid='.$_GET['menuid'].'&pc_hash='.$_GET['pc_hash'].'" class="layui-btn layui-btn-danger layui-btn-xs"><i class="fa fa-arrows"></i> '.L('remove','','content').'</a>';
					$array[] = $rs;
				}
			}
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			if(in_array($_POST['info']['parentid'],$arrchildid_arr,true)){
				showmessage(L('operation_failure'));
			}
	...
	function ismenu() {
		if(isset($_GET['dosubmit'])) {
			if (isset($_POST['ismenu'])) {
				$this->db->update(array('ismenu'=>$_POST['ismenu']),array('catid'=>$_POST['catid']));
			}
			$this->cache();
	...
	function disabled() {
		if(isset($_GET['dosubmit'])) {
			$row = $this->db->get_one(array('catid'=>$_POST['catid']));
			if (!$row) {
				dr_json(0, L('栏目数据不存在'));
			}
			...
			$this->content_db = pc_base::load_model('content_model');
			$this->categorys = getcache('category_content_'.$this->siteid,'commons');
			$modelid = $this->categorys[$_POST['catid']]['modelid'];
			if ($modelid) {
				$this->content_db->set_model($modelid);
				$table_name = $this->content_db->table_name;
			...
			if (isset($_POST['disabled'])) {
				$this->db->update(array('setting'=>array2string($row['setting'])),array('catid'=>$_POST['catid']));
			}
			$this->cache();
	...
	public function listorder() {
		if(isset($_GET['dosubmit'])) {
			if (isset($_POST['listorder'])) {
				$this->db->update(array('listorder'=>$_POST['listorder']),array('catid'=>$_POST['catid']));
			}
			$this->cache();
			dr_json(1, L('operation_success'));
		} else {
