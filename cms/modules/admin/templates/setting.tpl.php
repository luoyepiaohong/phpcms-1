...
<table width="100%"  class="table_form">
  ...
  <tr>
    <th width="120"><?php echo L('setting_attachment_file')?></th>
    <td class="y-bg">
      <div class="mt-radio-inline">
        <label class="mt-radio mt-radio-outline"><input type="radio" name="setconfig[attachment_file]" value="1"<?php echo ($attachment_file=='1') ? ' checked' : ''?>> <?php echo L('setting_yes');?> <span></span></label>
        <label class="mt-radio mt-radio-outline"><input type="radio" name="setconfig[attachment_file]" value="0"<?php echo ($attachment_file=='0') ? ' checked' : ''?>> <?php echo L('setting_no');?> <span></span></label>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo L('setting_attachment_file_desc')?>
      </div>
    </td>
  </tr>
  <tr>
...
<div id="div_setting_2" class="contentList pad-10 hidden">
  ...
  <tr>
    <th width="120"><?php echo L('后台登录验证码')?></th>
    <td class="y-bg">
	  <div class="mt-radio-inline">
  ...
  <tr>
    ...
    <td class="y-bg"><input type="text" class="input-text" name="setting[maxloginfailedtimes]" id="maxloginfailedtimes" size="10" value="<?php echo intval($maxloginfailedtimes)?>"/>后台登录失败N次后，系统将锁定登录，0表示不限制</td>
  </tr>
  <tr>
    <th><?php echo L('后台登录失败时限')?></th>
    <td class="y-bg"><input type="text" class="input-text" name="setting[sysadminlogintimes]" id="sysadminlogintimes" size="10" value="<?php echo intval($sysadminlogintimes)?>"/>单位分钟，后台登录失败锁定后在x分钟内禁止登录，默认10分钟</td>
  </tr>
  <tr>
    <th><?php echo L('安全密钥')?></th>
    <td class="y-bg"><input type="text" class="input-text" name="setconfig[auth_key]" id="auth_key" size="40" value="<?php echo $auth_key ? '************' : '';?>"/><button class="button" type="button" name="button" onclick="to_key()"> 重新生成 </button><br>密钥建议定期更换，更换密钥之后本次登录将会自动退出</td>
  </tr>
  <tr>
...
function to_key() {
   $.get('?m=admin&c=setting&a=syskey&pc_hash='+pc_hash, function(data){
		$('#auth_key').val(data);
	});
}
</script>
