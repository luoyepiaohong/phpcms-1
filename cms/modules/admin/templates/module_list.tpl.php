...
?>   
	...
	<td align="center">
	<?php if ($modules[$d]['iscore']) {?><span style="color: #999"><?php echo L('ban')?></span><?php } else {?><a href="javascript:void(0);" onclick="Dialog.confirm('<?php echo L('confirm', array('message'=>$modules[$d]['name']))?>',function(){uninstall('<?php echo $d?>');});"><font color="red"><?php echo L('unload')?></font></a><?php }?>
	</td>
	</tr>
<?php 
	} else {  
...
<!--
function install(id) {
	artdialog('install','?m=admin&c=module&a=install&module='+id,'<?php echo L('module_istall')?>',500,260);
}

function uninstall(id) {
	artdialog('install','?m=admin&c=module&a=uninstall&module='+id,'<?php echo L('module_unistall', '', 'admin')?>',500,260);
}
//-->
</script>
</body>
