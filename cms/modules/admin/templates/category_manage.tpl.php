...
<div class="admin-main layui-anim layui-anim-upbit">
    ...
    <blockquote class="layui-elem-quote">
        ...
        <a href="javascript:addedit('?m=admin&c=category&a=add&menuid=<?php echo$_GET['menuid'];?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>&s=1', '<?php echo L('add_page')?>')" class="layui-btn layui-btn-sm">
            <i class="fa fa-plus-square"></i> <?php echo L('add_page');?>
        </a>
        <a href="javascript:addedit('?m=admin&c=category&a=add&menuid=<?php echo$_GET['menuid'];?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>&s=2', '<?php echo L('add_cat_link')?>')" class="layui-btn layui-btn-sm">
            <i class="fa fa-plus-square-o"></i> <?php echo L('add_cat_link');?>
        </a>
        <a href="?m=admin&c=category&a=public_cache&menuid=<?php echo$_GET['menuid'];?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>&module=admin" class="layui-btn layui-btn-sm">
            <i class="fa fa-refresh"></i> <?php echo L('category_cache');?>
        </a>
        <a href="?m=admin&c=category&a=count_items&menuid=<?php echo$_GET['menuid'];?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>" class="layui-btn layui-btn-sm">
            <i class="fa fa-sort-amount-asc"></i> <?php echo L('count_items');?>
        </a>
        <a href="?m=admin&c=category&a=batch_edit&menuid=<?php echo$_GET['menuid'];?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>" class="layui-btn layui-btn-sm">
            <i class="fa fa-edit"></i> <?php echo L('category_batch_edit');?>
        </a>
        <a href="?m=content&c=sitemodel_field&a=init&menuid=<?php echo$_GET['menuid'];?>&modelid=-1&pc_hash=<?php echo $_SESSION['pc_hash'];?>" class="layui-btn layui-btn-sm">
            <i class="fa fa-bars"></i> <?php echo L('category_field_manage');?>
        </a>
        <a href="?m=content&c=sitemodel_field&a=init&menuid=<?php echo$_GET['menuid'];?>&modelid=-2&pc_hash=<?php echo $_SESSION['pc_hash'];?>" class="layui-btn layui-btn-sm">
...
<script type="text/html" id="url">
    ...
    {{# } else { }}
    <a href="?m=admin&c=category&a=public_cache&menuid=<?php echo $_GET['menuid'];?>&module=admin" class="layui-btn layui-btn-xs layui-btn-danger"><?php echo L('update_backup');?></a>
    {{# } }}
</script>
...
<script type="text/html" id="action">
    <a href="javascript:addedit('?m=admin&c=category&a=add&parentid={{d.id}}&menuid=<?php echo $_GET['menuid'];?>&s={{d.type}}&pc_hash=<?php echo $_GET['pc_hash'];?>', '<?php echo L('add_sub_category');?>')" class="layui-btn layui-btn-xs"><i class="fa fa-plus"></i> <?php echo L('add_sub_category');?></a>
    <a href="javascript:addedit('?m=admin&c=category&a=edit&catid={{d.id}}&menuid=<?php echo $_GET['menuid'];?>&type={{d.type}}&pc_hash=<?php echo $_GET['pc_hash'];?>', '<?php echo L('edit');?>')" class="layui-btn layui-btn-xs"><i class="fa fa-edit"></i> <?php echo L('edit');?></a>
    <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del"><i class="fa fa-trash-o"></i> <?php echo L('delete');?></a>
    <a href="?m=admin&c=category&a=remove&catid={{d.id}}&menuid=<?php echo $_GET['menuid'];?>&pc_hash=<?php echo $_GET['pc_hash'];?>" class="layui-btn layui-btn-danger layui-btn-xs"><i class="fa fa-arrows"></i> <?php echo L('remove','','content');?></a>
</script>
    ...
    layui.config({
        ...
        ptable=treeGrid.render({
            ...
            ,cols: [[
                ...
                {field: 'manage',title: '<?php echo L('operations_manage');?>',width: 325,align: 'center', toolbar: '#action', fixed: 'right'}
            ]]
            ,page:false
        });
