...
class attachment extends admin {
	...
	public function save() {
		$this->set_config($_POST['data']);	 //保存进config文件
		$this->setcache();
	...
	public function set_config($config, $filename="system") {
		...
		foreach($config as $k=>$v) {
			if(in_array($k,array('sys_attachment_attach','sys_attachment_path','sys_attachment_save_type','sys_attachment_save_dir','sys_attachment_url','sys_avatar_path','sys_avatar_url','sys_thumb_path','sys_thumb_url','attachment_stat'))) {
				$v = trim($v);
				$configs[$k] = $v;
