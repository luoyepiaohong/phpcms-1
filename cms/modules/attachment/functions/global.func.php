<?php
	...
	function file_icon($file,$type = 'png') {
		...
		} elseif (is_file(CMS_PATH.IMG_PATH.'ext/'.$ext.'.png')) {
				...
				upload.render({
					...
					before: function (obj) {
						...
						if(data.code == 1){
							...
							if(data.ext == 1) {
								var img = '<span class=\"checkbox\"></span><input type=\"checkbox\" class=\"checkboxes\" name=\"ids[]\" value=\"'+data.id+'\" /><a href=\"javascript:;\" onclick=\"javascript:att_cancel(this,'+data.id+',\'upload\')\" class=\"on\"><div class=\"icon\"></div><img src=\"'+data.src+'\" width=\"80\" height=\"150\" id=\"'+data.id+'\" path=\"'+data.src+'\" filename=\"'+data.filename+'\"/><i class=\"size\">'+data.size+'</i><i class=\"name\" title=\"'+data.filename+'\">'+data.filename+'</i></a>';
							} else {
								var img = '<a href=\"javascript:;\" onclick=\"javascript:att_cancel(this,'+data.id+',\'upload\')\" class=\"on\"><div class=\"icon\"></div><img src=\"".IMG_PATH."ext/'+data.ext+'.png\" width=\"80\" id=\"'+data.id+'\" path=\"'+data.src+'\" filename=\"'+data.filename+'\"/></a>';
							}
